  ...
  const acceptedRosterList = useMemo(() => {
    ...
    candidates.forEach((candidate) => {
      ...
      (candidate.contractDeliverables || []).forEach((contract) => {
        const siteAddress =
          contract.siteAddress || contract.siteName || "N/A";
        const scopeOfWork = contract.scopeOfWork || "";

        (contract.services || []).forEach((service) => {
          (service.assignedEmployees || []).forEach((slot, slotIdx) => {
            const empName = slot.employee || (slotIdx === 0 ? service.employee : "");

            if (empName && empName.trim() !== "" && slot.approvalState === "Accepted") {
              list.push({
    ...
    candidates.forEach((candidate) => {
      ...
      (candidate.contractDeliverables || []).forEach((contract) => {
        ...
        (contract.adhocServices || []).forEach((adhoc, aIdx) => {
          ...
          if (empName && empName.trim() !== "" && adhoc.approvalState === "Accepted") {
            list.push({
      ...
      <div className="accountsHeader">
        <div>
          ...
          <p className="accountsSubtext">
            View accepted roster employee shifts and accepted adhoc services.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="accountsTableWrapper">
          {filteredAdhoc.length === 0 ? (
            <div className="accountsEmpty">No Accepted Adhoc Services Found</div>
          ) : (
            <table className="accountsTable">
              <thead>
                <tr>
